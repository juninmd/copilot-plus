...
export class QuotaService {
  ...
  public async fetchQuota(forceRefresh = false): Promise<QuotaResult> {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        const body = await this.httpGet(TOKEN_URL, token);
        const parsed = JSON.parse(body) as { token?: string };
        if (!parsed.token) {
          this.logger.log('API response missing "token" field');
          break;
        }
        const payload = this.decodeJwtPayload(parsed.token);
        const quota = this.extractQuota(payload, configTotal);
        if (quota) {
        ...
        this.logger.log(`Attempt ${attempt + 1} failed: ${String(err)}`);
  ...
  private async getGitHubToken(): Promise<string | null> {
    try {
      ...
      if (session) {
        this.logger.log(`Auth: signed in as ${session.account.label}`);
        return session.accessToken;
      }
    } catch (err) {
      this.logger.log(`Auth error: ${String(err)}`);
    }
    this.logger.log('Auth: No GitHub session found. Sign in via VS Code → Accounts.');
    return null;
  }
  ...
  private async httpGet(url: string, token: string): Promise<string> {
    ...
    this.logger.log(`HTTP ${res.status} ← ${url} | ct=${contentType} | ce=${contentEncoding} | te=${transferEncoding}`);
    ...
    this.logger.log(`Body: ${bytes.length} bytes | first bytes hex: ${hexPreview}`);
    ...
    this.logger.log(`Body text (first 400): ${body.slice(0, 400)}`);
    return body;
  }
  ...
  private extractQuota(payload: JwtPayload, configTotal: number): QuotaInfo | null {
    this.logger.log(`JWT keys: [${Object.keys(payload).join(', ')}] | sku: ${payload.sku ?? 'n/a'}`);
    ...
    this.logger.log(`No quota fields found. Full payload: ${JSON.stringify(payload, null, 0).slice(0, 500)}`);
    return null;
  }
}
